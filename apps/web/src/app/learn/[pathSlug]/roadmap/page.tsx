...
import { fetchPathRoadmap } from '@/lib/api';
import type { PathRoadmapData } from '@/lib/api';
import { Loader2, Map, List, ArrowLeft } from 'lucide-react';
    ...
    return (
      <div className="flex h-full items-center justify-center bg-[--bg-primary]">
        <div className="text-center">
          <p className="text-[--text-primary] font-semibold mb-2">Roadmap unavailable</p>
          <p className="text-[--text-muted] text-sm mb-4">Could not load the roadmap for <strong>{pathSlug}</strong>.</p>
          <Link href="/dashboard" className="text-[--accent-ai] hover:underline text-sm">← Back to Dashboard</Link>
        </div>
      </div>
    );
  }
