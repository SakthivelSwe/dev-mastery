  ...
  "steps": [
    {
      ...
      "description": "Arrays are ordered lists. Access by 0-based index. length gives count. Negative indices not supported natively (use at(-1) for last).",
      ...
      "diagram": {
        ...
        "items": [
          ...
          {
            "label": "arr.at(-1) = 5 (last)",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "length: O(1) — stored property",
            "color": "#818cf8"
          }
        ]
      }
    },
    {
      ...
      "description": "push/pop: add/remove from end. unshift/shift: add/remove from front. splice: general insert/remove. sort: in-place sort.",
      "code": "const a = [1,2,3];\na.push(4);         // [1,2,3,4] — O(1)\na.pop();           // [1,2,3]   — O(1)\na.unshift(0);      // [0,1,2,3] — O(n) shift!\na.shift();         // [1,2,3]   — O(n)\na.splice(1,1,9);   // [1,9,3]   — remove 1 at idx 1, insert 9",
      "diagram": {
        "kind": "boxes",
        "title": "Mutating method cost",
        "items": [
          {
            "label": "push/pop: O(1) — end",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "shift/unshift: O(n) — shifts all",
            "color": "#f59e0b"
          },
          {
            "label": "splice: O(n) — general",
            "color": "#f59e0b"
          }
        ]
      }
    },
    {
      ...
      "description": "map, filter, reduce, find, slice, concat, flat, flatMap — all return NEW arrays without modifying the original.",
      "code": "const nums = [1,2,3,4,5];\nnums.map(x => x*2);        // [2,4,6,8,10]\nnums.filter(x => x%2===0); // [2,4]\nnums.reduce((s,x)=>s+x,0); // 15\nnums.slice(1,3);            // [2,3]\n[[1,2],[3,4]].flat();       // [1,2,3,4]\nnums.flatMap(x => [x,x*2]); // [1,2,2,4,3,6,4,8,5,10]",
      "diagram": {
        "kind": "flow",
        "steps": [
          {
            "label": "original array UNCHANGED",
            "done": true
          },
          {
            "label": "map/filter/reduce return new array",
            "done": true
          },
          {
            "label": "chain: arr.filter(...).map(...)...",
            "active": true
          }
        ]
      }
    },
    {
      ...
      "description": "Destructuring extracts values by position. Spread creates a shallow copy or expands into a new array.",
      ...
      "diagram": {
        "kind": "boxes",
        "title": "Destructure + spread",
        "items": [
          {
            "label": "[a, b, ...rest]: extract by position",
            "color": "#818cf8"
          },
          {
            "label": "[...arr]: shallow copy",
            "color": "#10b981",
            "highlight": true
          },
          {
    ...
    },
    {
      ...
      "description": "indexOf/lastIndexOf: find by value. includes: boolean. find/findIndex: by predicate. sort: in-place, string by default! Always pass a comparator.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": ".sort() without comparator: lexicographic",
            "color": "#ef4444"
          },
          {
            "label": ".sort((a,b)=>a-b): numeric ascending",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": ".sort((a,b)=>b-a): numeric descending",
            "color": "#818cf8"
          }
        ]
      }
