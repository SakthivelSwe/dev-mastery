...
```json
{
  ...
  "steps": [
    {
      "title": "Proxy — intercept object operations",
      "description": "Proxy wraps an object with traps for get, set, has, deleteProperty, and more. The target is the original object.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "Proxy",
            "to": "handler",
            "label": "get trap(target, \"x\")"
          },
          {
            "from": "handler",
            "to": "target",
            "label": "Reflect.get(target, \"x\")"
          },
          {
            "from": "target",
            "to": "code",
    ...
    },
    {
      ...
      "description": "Intercept set operations to enforce validation rules. Throw before invalid data reaches the object.",
      ...
      "diagram": {
        ...
        "steps": [
          ...
          {
            "label": "set trap intercepts",
            "done": true
          },
          {
            "label": "schema check fails → TypeError",
            "active": true
          },
          {
    ...
    {
      "title": "Observable state via Proxy",
      "description": "Proxy powers Vue 3 reactivity. Track access (get) and mutations (set) to drive re-renders.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "get trap: track() dependency",
            "color": "#818cf8"
          },
          {
            "label": "set trap: trigger() effects",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "component re-renders on tracked change",
            "color": "#10b981"
          }
        ]
      }
    ...
    {
      "title": "Reflect — mirror of Proxy traps",
      "description": "Reflect has the same methods as Proxy traps. Provides a clean API for default behavior.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "mirrors all Proxy trap operations",
            "color": "#818cf8"
          },
          {
            "label": "use in traps for default behavior",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "cleaner than Fn.call / manual apply",
            "color": "#818cf8"
          }
        ]
      }
    },
    {
      ...
      "description": "Proxy.revocable() creates a proxy that can be permanently disabled. Useful for temporary access or capability revocation.",
      "code": "const { proxy, revoke } = Proxy.revocable({ secret: 42 }, {});\nproxy.secret;   // 42\nrevoke();\nproxy.secret;   // TypeError: Cannot perform 'get' on revoked Proxy",
      "diagram": {
        "kind": "flow",
        "steps": [
          {
            "label": "proxy created with revoke handle",
            "done": true
          },
          {
            "label": "access works normally",
            "done": true
          },
          {
            "label": "revoke() permanently disables proxy",
            "active": true
          },
          {
