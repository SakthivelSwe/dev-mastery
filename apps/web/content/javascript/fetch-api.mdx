...
```json
{
  ...
  "steps": [
    {
      "title": "fetch — modern HTTP client",
      "description": "fetch() returns a Promise that resolves to a Response. You must call .json(), .text() etc. to parse the body.",
      "code": "const response = await fetch(\"/api/users\");\nif (!response.ok) throw new Error(`HTTP ${response.status}`);\nconst users = await response.json();\n// response.ok: status 200-299\n// response.status: 200, 404, 500...",
      "diagram": {
        "kind": "flow",
        "steps": [
          {
            "label": "fetch() → Promise<Response>",
            "done": true
          },
          {
            "label": "check response.ok (200-299)",
            "done": true
          },
          {
            "label": "response.json() → Promise<data>",
            "active": true
          }
        ]
      }
    },
    {
      ...
      "description": "Send a POST request with JSON. Set Content-Type header. Stringify the body.",
        ...
        "messages": [
          {
            "from": "client",
            "to": "fetch",
            "label": "POST /api/users + JSON body"
          },
          {
            "from": "fetch",
            "to": "server",
    ...
    },
    {
      ...
      "description": "fetch does NOT reject on HTTP errors (404, 500). Must check response.ok. Only network failures reject the Promise.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "fetch rejects: network error only",
            "color": "#f59e0b"
          },
          {
            "label": "404/500: response.ok = false, must check!",
            "color": "#ef4444",
            "highlight": true
          },
          {
            "label": "always throw on !response.ok",
            "color": "#10b981"
          }
        ]
      }
    },
    {
      ...
      "description": "AbortController lets you cancel a fetch. Use for debouncing search, navigation away, or timeout.",
      "code": "const controller = new AbortController();\n\nconst response = await fetch(url, {\n    signal: controller.signal\n});\n\n// Cancel after 5 seconds\nconst timeout = setTimeout(() => controller.abort(), 5000);\n\n// Cancel on navigation\nuseEffect(() => {\n    return () => controller.abort();\n}, []);",
      "diagram": {
        "kind": "flow",
        "steps": [
          {
            "label": "create AbortController",
            "done": true
          },
          {
            "label": "pass signal to fetch",
            "done": true
          },
          {
            "label": "call abort() to cancel",
            "active": true
          },
          {
    ...
    {
      "title": "fetch with headers and auth",
      "description": "Pass authentication tokens, custom headers, CORS credentials.",
      "code": "const token = localStorage.getItem(\"token\");\n\nconst res = await fetch(\"/api/protected\", {\n    headers: {\n        \"Authorization\": `Bearer ${token}`,\n        \"Accept\": \"application/json\",\n        \"X-Request-ID\": crypto.randomUUID()\n    },\n    credentials: \"include\",  // send cookies\n});",
      "diagram": {
        "kind": "boxes",
        "title": "Headers patterns",
        "items": [
          {
          ...
          {
            "label": "Content-Type: required for POST body",
            "color": "#818cf8"
          }
        ]
      }
