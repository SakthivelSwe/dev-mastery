  ...
  "steps": [
    {
      ...
      "description": "Load data without page refresh. XMLHttpRequest was the original API (1999). Now superseded by fetch(), but XHR still used for file uploads.",
        ...
        "messages": [
          {
            "from": "page",
            "to": "XHR",
            "label": "xhr.open() + xhr.send()"
          },
          {
            "from": "XHR",
            "to": "server",
            "label": "HTTP GET /api/data"
          },
          {
            "from": "server",
            "to": "XHR",
    ...
    {
      "title": "XHR file upload with progress",
      "description": "XHR supports upload progress events. Useful for file upload progress bars.",
      "code": "function uploadFile(file) {\n    const xhr = new XMLHttpRequest();\n    const formData = new FormData();\n    formData.append(\"file\", file);\n    xhr.upload.onprogress = e => {\n        const pct = (e.loaded / e.total * 100).toFixed(0);\n        progressBar.value = pct;\n    };\n    xhr.onload = () => console.log(\"Done:\", xhr.status);\n    xhr.open(\"POST\", \"/upload\");\n    xhr.send(formData);\n}",
      "diagram": {
        "kind": "flow",
        "steps": [
          {
            "label": "xhr.upload.onprogress: bytes sent",
            "done": true
          },
          {
            "label": "e.loaded / e.total: percentage",
            "done": true
          },
          {
            "label": "update progress bar in real time",
            "active": true
          }
        ]
      }
    ...
    {
      "title": "fetch vs XHR comparison",
      "description": "fetch: Promises-based, cleaner API, no upload progress (use XHR for that). XHR: callback-based, supports upload progress.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "fetch: Promises, clean, default choice",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "XHR: upload progress event",
            "color": "#818cf8"
          },
          {
            "label": "Axios: wraps XHR with better API",
            "color": "#818cf8"
          }
        ]
      }
    },
    {
      ...
      "description": "Browser blocks cross-origin requests unless server sends CORS headers. Preflight OPTIONS request for non-simple requests.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "server",
            "to": "browser",
            "label": "200 Access-Control-Allow-*"
          },
          {
            "from": "browser",
            "to": "server",
            "label": "actual POST request"
          }
        ]
      }
    },
    {
      "title": "Axios — HTTP client library",
      "description": "Axios wraps XHR/fetch with interceptors, auto JSON, timeout, request cancellation.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "interceptors: auth/logging centralized",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "auto JSON: no .json() needed",
            "color": "#10b981"
          },
          {
            "label": "timeout: built-in",
            "color": "#818cf8"
          }
        ]
      }
