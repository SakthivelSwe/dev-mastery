...
```json
{
  ...
  "steps": [
    {
      "title": "REST CRUD mapping",
      "description": "Create → POST /resources. Read → GET /resources/{id}. Update → PUT /resources/{id}. Delete → DELETE /resources/{id}.",
    ...
    },
    {
      ...
      "description": "Controller calls Service; Service applies business logic and calls Repository; Repository talks to DB.",
        ...
        "messages": [
          {
            "from": "Controller",
            "to": "UserService",
            "label": "createUser(dto)"
          },
          {
            "from": "UserService",
            "to": "UserRepository",
            "label": "existsByEmail + save"
          },
          {
            "from": "UserRepository",
            "to": "DB",
    ...
    {
      "title": "Input validation with Bean Validation",
      "description": "@Valid on @RequestBody triggers Bean Validation. Annotate DTO fields with @NotNull, @Size, @Email. Validation failures produce 400 Bad Request.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "@RequestBody UserDTO received",
            "done": true
          },
          {
          ...
          {
            "label": "Constraint violations? → 400 Bad Request",
            "active": true
          },
          {
    ...
    },
    {
      ...
      "description": "Extend JpaRepository. get CRUD for free. Add custom queries via method names or @Query.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "extends JpaRepository → CRUD generated",
            "done": true
          },
          {
            "label": "findByEmail → SELECT WHERE email=?",
            "done": true
          },
          {
            "label": "findByActiveTrue(pageable) → paginated SELECT",
            "active": true
          }
        ]
      }
    ...
    {
      "title": "Error handling and HTTP status codes",
      "description": "Return proper HTTP codes: 200 (ok), 201 (created), 204 (no content for delete), 400 (validation), 404 (not found), 409 (conflict), 500 (server error).",
