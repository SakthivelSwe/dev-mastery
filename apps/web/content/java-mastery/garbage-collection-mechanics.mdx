...
```json
{
  ...
  "steps": [
    {
      "title": "Heap regions in G1GC",
      "description": "G1GC divides the heap into equal-size regions (1–32 MB). Each region is labelled Eden, Survivor, Old, or Humongous. G1 chooses which regions to collect based on a pause-time goal.",
      "code": "// Start JVM with G1GC (default Java 9+)\njava -XX:+UseG1GC -Xmx4g -Xms4g\n     -XX:MaxGCPauseMillis=200   // pause-time goal\n     -XX:G1HeapRegionSize=16m",
      "diagram": {
        "kind": "boxes",
        "title": "G1GC heap regions",
        "items": [
          {
            "label": "Eden (young)",
            "color": "#10b981",
    ...
    {
      "title": "Minor GC — eden to survivor",
      "description": "When Eden fills, live objects are copied to a Survivor space. Short-lived objects (most Java objects) die here and are never promoted.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "Eden fills up → Minor GC fires",
            "done": true
          },
          {
            "label": "Trace live objects from roots",
            "done": true
          },
          {
            "label": "Copy live → Survivor space (compact)",
            "active": true
          },
          {
    ...
    {
      "title": "Object promotion — tenuring threshold",
      "description": "Each GC, an object's age counter increments. When age reaches the tenuring threshold (default 15), the object is promoted to Old Gen.",
      ...
      "diagram": {
        ...
        "steps": [
          ...
          {
            "label": "survives GC → Survivor, age+1",
            "done": true
          },
          {
            "label": "age reaches threshold (15)",
            "active": true
          },
          {
    ...
    {
      "title": "G1 mixed collection",
      "description": "When Old Gen reaches a threshold (~45%), G1 marks live objects (concurrent marking) then performs mixed collections — collecting both Young AND selected Old regions.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "Initial Mark (Stop-the-World, < 10ms)",
            "done": true
          },
          {
            "label": "Concurrent marking — app runs",
            "done": true
          },
          {
            "label": "Remark (STW)",
          ...
          {
            "label": "Mixed collections: collect Old regions",
            "active": true
          }
        ]
      }
    },
    {
      ...
      "description": "Key flags: heap size, region size, pause goal, GC log. Always measure before tuning.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "-Xms = -Xmx",
            "value": "no resize pauses",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "MaxGCPauseMillis",
            "value": "pause-time goal (not guarantee)",
            "color": "#818cf8"
          },
          {
            "label": "-Xlog:gc*",
            "value": "GC logging for analysis",
            "color": "#f59e0b"
          }
        ]
      }
