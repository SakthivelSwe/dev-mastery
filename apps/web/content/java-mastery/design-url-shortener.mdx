...
{
  "language": "java",
  "fileName": "UrlShortener.java",
  "steps": [
    {
      "title": "Requirements and estimations",
      "description": "Input: long URL. Output: 6-char short code (62^6 = 56 billion combos). 100M writes/day = 1160/s. 10B reads/day = 115K/s. Read-heavy.",
      ...
      "diagram": {
        ...
        "items": [
          ...
          {
            "label": "reads",
            "value": "115k/s (100× more)",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "strategy: optimize for reads",
            "color": "#10b981"
          }
        ]
      }
    },
    {
      ...
      "description": "Use a counter + base62 encode OR hash the long URL. Counter gives sequential IDs. Hash gives distributed generation but needs collision check.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "IDGen",
            "to": "API",
            "label": "long id → base62 → \"abc12Z\""
          },
          {
            "from": "API",
            "to": "DB",
            "label": "INSERT (abc12Z, longUrl)"
          }
        ]
      }
    },
    {
      "title": "Redirect flow",
      "description": "GET /{code} → lookup in cache → if miss, query DB → 301 Permanent Redirect (browsers cache) or 302 Temporary (analytics accurate).",
      ...
      "diagram": {
        ...
        "steps": [
          ...
          {
            "label": "cache hit? → 302 redirect (< 1ms)",
            "done": true
          },
          {
            "label": "cache miss → DB lookup → cache → 302",
            "active": true
          }
        ]
      }
    },
    {
      ...
      "description": "Cache hot URLs in Redis (most popular 20% served from cache 80% of time). Use LRU eviction. TTL matching URL expiry.",
      ...
      "diagram": {
        ...
        "items": [
          ...
          },
          {
            "label": "DB (Postgres)",
            "color": "#f59e0b",
            "value": "~10ms, source of truth"
          }
        ]
      }
    },
    {
      "title": "Analytics and scalability",
      "description": "Track click counts with Redis incr. Scale writes with a Kafka queue (producer publishes, consumer batch-inserts). Scale reads with read replicas.",
        ...
        "messages": [
          {
            "from": "API",
            "to": "Redis",
            "label": "INCR clicks:abc12Z"
          },
          {
            "from": "API",
            "to": "Kafka",
            "label": "publish ClickEvent"
          },
          {
            "from": "Kafka",
            "to": "AnalyticsDB",
            "label": "batch insert (async)"
          }
        ]
      }
    }
