...
```json
{
  ...
  "steps": [
    {
      "title": "Distributed transactions problem",
      "description": "You can't use a single DB transaction across multiple services (each owns its own DB). The Saga pattern provides eventual consistency without distributed locks.",
      "code": "// Order saga: place order → reserve inventory → charge payment\n// Each step has a compensating transaction if later steps fail",
      "diagram": {
        "kind": "flow",
        "steps": [
          {
            "label": "createOrder → OrderCreated event",
            "done": true
          },
          {
            "label": "reserveInventory → InventoryReserved",
            "done": true
          },
          {
            "label": "chargePayment → SUCCESS or FAIL",
            "active": true
          },
          {
    ...
    },
    {
      ...
      "description": "Each service listens for events and publishes its own. No central coordinator. Services react to events autonomously.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "inventory-svc",
            "to": "payment-svc",
            "label": "InventoryReserved event"
          },
          {
            "from": "payment-svc",
            "to": "order-svc",
            "label": "PaymentProcessed event"
          }
        ]
      }
    },
    {
      ...
      "description": "A central saga orchestrator commands each service step by step. Easier to track state and handle failures.",
      ...
      "diagram": {
        "kind": "boxes",
        "title": "Orchestrator vs Choreography",
        "items": [
          {
            "label": "Choreography: decentralized",
            "color": "#818cf8"
          },
          {
            "label": "Orchestrator: central controller",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "Orchestrator: easier to track state",
            "color": "#10b981"
          }
        ]
      }
    },
    {
      ...
      "description": "Each saga step has a compensating action. If step N fails, execute compensating actions for steps N-1 down to 1.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "forward: create → reserve → charge",
            "done": true
          },
          {
            "label": "charge FAILS",
          ...
          {
            "label": "compensate: refund → release → cancel",
            "active": true
          }
        ]
      }
    },
    {
      ...
      "description": "Track saga state to handle restarts and partial failures. Store state in a saga table.",
