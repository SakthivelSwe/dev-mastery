...
{
  "language": "java",
  "fileName": "MsArch.java",
  "steps": [
    {
      "title": "Typical microservices stack",
      "description": "Gateway → Services → Message Bus → Databases. Infrastructure: service registry, config server, distributed tracing.",
    ...
    {
      "title": "Service registry — Eureka / Consul",
      "description": "Services register themselves on startup. Other services discover endpoints by name, not hardcoded IP.",
        ...
        "messages": [
          {
            "from": "user-service",
            "to": "Eureka",
            "label": "register: user-service@10.0.0.5:8080"
          },
          {
            "from": "order-service",
            "to": "Eureka",
            "label": "lookup: where is user-service?"
          },
          {
            "from": "Eureka",
            "to": "order-service",
    ...
    {
      "title": "Config server — centralized config",
      "description": "Spring Cloud Config Server externalizes configuration. Services pull config from Git on startup and refresh dynamically.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "config-server reads Git repo",
            "done": true
          },
          {
            "label": "order-service starts: GET /order-service/prod",
            "done": true
          },
          {
            "label": "receives merged config",
            "active": true
          },
          {
    ...
    },
    {
      ...
      "description": "If user-service is down, order-service should fail fast instead of waiting. Resilience4j circuit breaker opens after threshold failures.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "CLOSED: call user-service",
            "done": true
          },
          {
            "label": "failures > threshold → OPEN",
            "done": true
          },
          {
            "label": "OPEN: return fallback immediately",
            "active": true
          },
          {
    ...
    {
      "title": "Distributed tracing — Zipkin / Jaeger",
      "description": "Each request gets a trace ID propagated through all services via HTTP headers. Visualize the full request path across services.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "gateway",
            "to": "order-svc",
            "label": "route [traceId=abc, spanId=001]"
          },
          {
            "from": "order-svc",
            "to": "user-svc",
            "label": "call [traceId=abc, spanId=002]"
          },
          {
            "from": "user-svc",
            "to": "Zipkin",
