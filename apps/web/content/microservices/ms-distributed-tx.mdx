...
```json
{
  ...
  "steps": [
    {
      "title": "Why 2PC fails in microservices",
      "description": "Two-Phase Commit (2PC) requires all participants to hold locks until the coordinator decides. In microservices, this creates tight coupling, blocking, and can't tolerate network partitions.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "locks held across network calls",
            "color": "#ef4444",
            "highlight": true
          },
          {
            "label": "coordinator crash → deadlock",
            "color": "#ef4444"
          },
          {
            "label": "violates CAP: choose A or C in partition",
            "color": "#ef4444"
          }
        ]
      }
    ...
    {
      "title": "Eventual consistency is the goal",
      "description": "Accept that distributed systems can only guarantee eventual consistency. Design for it: make operations idempotent, use compensating transactions.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "order created → immediately visible",
            "done": true
          },
          {
            "label": "payment processing → async",
            "done": true
          },
          {
            "label": "payment success → order confirmed",
            "active": true
          },
          {
    ...
    {
      "title": "Saga pattern recap",
      "description": "A saga is a sequence of local transactions, each publishing an event/message to trigger the next step. On failure, compensating transactions undo prior steps.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "step 1: create order",
            "color": "#818cf8"
          },
          {
            "label": "step 2: reserve inventory",
            "color": "#818cf8"
          },
          {
            "label": "step 3 fail: refund → release → cancel",
            "color": "#ef4444",
            "highlight": true
          }
        ]
    ...
    },
    {
      ...
      "description": "Each service exposes try (reserve), confirm (commit), cancel (rollback). Coordinator calls try on all, then confirm or cancel.",
        ...
        "messages": [
          {
            "from": "coordinator",
            "to": "inventory",
            "label": "try: reserve(orderId)"
          },
          {
            "from": "coordinator",
            "to": "payment",
            "label": "try: hold(orderId)"
          },
          {
            "from": "coordinator",
            "to": "inventory",
    ...
    {
      "title": "When to accept eventual consistency",
      "description": "Most user-facing operations can tolerate eventual consistency. Use immediate consistency only where legally or contractually required.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "eventual OK: orders, notifications",
            "color": "#10b981"
          },
          {
            "label": "strict: financial, legal compliance",
            "color": "#ef4444",
            "highlight": true
          },
          {
            "label": "default: design for eventual consistency",
            "color": "#10b981"
          }
        ]
      }
