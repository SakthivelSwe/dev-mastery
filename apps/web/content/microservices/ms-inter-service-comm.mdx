...
{
  "language": "java",
  "fileName": "InterServiceComm.java",
  "steps": [
    {
      "title": "Synchronous: REST + Feign",
      "description": "Service A calls Service B via HTTP and waits for the response. Simple, but creates coupling — if B is slow/down, A is affected.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "user-service",
            "to": "order-service",
            "label": "200 { \"id\":42, \"name\":\"Alice\" }"
          }
        ]
      }
    },
    {
      "title": "Synchronous: gRPC for internal services",
      "description": "gRPC uses HTTP/2 + Protobuf. 3-10× faster than REST. Strong typing via .proto schema. Excellent for internal microservice calls.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "REST: HTTP/1.1 + JSON, human-readable",
            "color": "#818cf8"
          },
          {
            "label": "gRPC: HTTP/2 + Protobuf, 3-10× faster",
            "color": "#10b981",
            "highlight": true
          },
          {
    ...
    {
      "title": "Asynchronous: Kafka events",
      "description": "Service A publishes an event; Service B consumes it at its own pace. Decoupled, resilient. B can be down; events wait.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            ...
            "label": "deliver OrderPlaced (when ready)"
          }
        ]
      }
    },
    {
      ...
      "description": "A service mesh handles cross-cutting communication: mTLS (encryption), retries, circuit breaking, observability — without code changes.",
      "code": "# Istio sidecar (Envoy proxy) injected automatically\n# Handles:\n# - mTLS between all services\n# - Automatic retries\n# - Circuit breaking\n# - Request tracing\n# Your code sees: plain HTTP; Istio handles the rest",
      "diagram": {
        "kind": "boxes",
        "title": "Service mesh capabilities",
        "items": [
          {
            "label": "mTLS: automatic encryption",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "retries + circuit breaking: no code change",
            "color": "#10b981"
          },
          {
            "label": "traffic management: canary, A/B",
            "color": "#818cf8"
          },
          {
            "label": "observability: traces, metrics auto",
            "color": "#818cf8"
          }
        ]
      }
    ...
    {
      "title": "Choosing sync vs async",
      "description": "Sync: when you need the result immediately (user lookup during order validation). Async: when the result isn't needed now (send email after order placed).",
      ...
      "diagram": {
        "kind": "boxes",
        "title": "Sync vs Async choice",
        "items": [
          {
            "label": "need result immediately → sync (REST/gRPC)",
            "color": "#818cf8"
          },
          {
            "label": "fire and forget → async (Kafka)",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "default to async where possible",
            "color": "#10b981"
          }
        ]
      }
