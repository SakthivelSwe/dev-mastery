...
```json
{
  ...
  "steps": [
    {
      "title": "Three states: CLOSED → OPEN → HALF-OPEN",
      "description": "CLOSED: normal operation. OPEN: fail fast after failure threshold. HALF-OPEN: probe if service recovered.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "CLOSED: normal calls",
            "done": true
          },
          {
            "label": "failure rate > threshold → OPEN",
            "done": true
          },
          {
            "label": "OPEN: fail-fast, no calls made",
            "active": true
          },
          {
    ...
    },
    {
      ...
      "description": "Configure sliding window, failure rate threshold, wait duration in OPEN state, and permitted calls in HALF-OPEN.",
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "sliding-window-size: 10 calls",
            "color": "#818cf8"
          },
          {
            "label": "failure-rate-threshold: 50%",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "wait-duration: 30s OPEN",
            "color": "#f59e0b"
          },
          {
            "label": "half-open test calls: 3",
            "color": "#818cf8"
          }
        ]
      }
    },
    {
      ...
      "description": "When circuit is OPEN or call throws, the fallback method is invoked. Provide a meaningful degraded response.",
      ...
      "diagram": {
        ...
        "messages": [
          ...
          },
          {
            "from": "CB",
            "to": "caller",
            "label": "OPEN → fallbackMethod called"
          },
          {
            "from": "CB",
            "to": "fallback",
            "label": "inventoryFallback(42, ex)"
          },
          {
            "from": "fallback",
            "to": "caller",
            "label": "Stock.unknown(42)"
          }
        ]
      }
    },
    {
      "title": "Circuit breaker with Feign client",
      "description": "Combine Spring Cloud OpenFeign with Resilience4j for automatic circuit breaking on inter-service calls.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "Feign calls user-service",
            "done": true
          },
          {
            "label": "exception or CB open",
            "done": true
          },
          {
            "label": "UserClientFallback.getUser() invoked",
            "active": true
          }
        ]
      }
    ...
    {
      "title": "Bulkhead — isolate failure domains",
      "description": "Bulkhead limits concurrent calls to each downstream service. If one service is slow, it can't consume all threads.",
