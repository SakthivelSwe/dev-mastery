  ...
  "steps": [
    {
      ...
      "description": "Saving to DB and publishing to Kafka in the same operation can fail partially — DB saved but Kafka not published (or vice versa). The outbox pattern solves this.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "save order to DB ✓",
            "done": true
          },
          {
            "label": "publish to Kafka ✗ (network down)",
            "done": true
          },
          {
            "label": "order saved but event LOST",
            "active": true
          },
          {
    ...
    {
      "title": "Outbox table",
      "description": "Write the event to an outbox table IN THE SAME TRANSACTION as the business data. Atomicity guaranteed by the DB transaction.",
      ...
      "diagram": {
        ...
        "items": [
          ...
          {
            "label": "save outbox record",
            "color": "#818cf8"
          },
          {
            "label": "BOTH in one transaction → atomic",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "Kafka publish is async/separate",
            "color": "#818cf8"
          }
        ]
      }
    ...
    {
      "title": "Relay process",
      "description": "A relay (poller or CDC listener) reads the outbox table and publishes to Kafka. Marks records as published after success.",
        ...
        "messages": [
          {
            "from": "relay",
            "to": "outbox table",
            "label": "SELECT WHERE published=false"
          },
          {
            "from": "relay",
            "to": "Kafka",
            "label": "send event payload"
          },
          {
            "from": "relay",
            "to": "outbox table",
            "label": "UPDATE published=true"
          }
        ]
      }
    },
    {
      "title": "Transactional outbox vs Debezium CDC",
      "description": "Simple poller has at-least-once delivery. Debezium reads Postgres WAL (change data capture) — zero-latency, no polling, exact delivery.",
      ...
      "diagram": {
        "kind": "boxes",
        "title": "Relay approaches",
        "items": [
          {
            "label": "polling relay: simple, ~1s delay",
            "color": "#818cf8"
          },
          {
            "label": "Debezium CDC: real-time, reads WAL",
            "color": "#10b981",
            "highlight": true
          },
          {
            "label": "both guarantee at-least-once delivery",
            "color": "#818cf8"
          }
        ]
      }
    },
    {
      ...
      "description": "The outbox guarantees at-least-once. Consumers MUST be idempotent — processing the same event twice has no additional effect.",
