...
```json
{
  "steps": [
    {
      "title": "Custom Pipe",
      "description": "Pipes transform data in templates — implement PipeTransform interface.",
    ...
    },
    {
      ...
      "diagram": {
        ...
        "items": [
          {
            "label": "Pure: ref change only",
            "color": "#43a047"
          },
          {
            "label": "Impure: every CD cycle",
            "color": "#e53935"
          }
        ]
      }
    ...
    {
      "title": "Built-in Pipes Deep Dive",
      "description": "DatePipe, CurrencyPipe, DecimalPipe, PercentPipe — powerful formatting options.",
      "code": "{{ 1234567.89 | number:'1.2-2' }}\n// → '1,234,567.89'\n\n{{ 0.1234 | percent:'1.1-2' }}\n// → '12.34%'\n\n{{ 9999.99 | currency:'EUR':'symbol':'1.2-2':'de' }}\n// → '9.999,99 €' (German locale)\n\n{{ now | date:'EEEE, MMMM d, y' }}\n// → 'Monday, July 4, 2026'\n\n{{ 'hello world' | titlecase }}\n// → 'Hello World'\n\n{{ users | slice:0:5 | json }}",
      "highlight": [
        1,
        4,
        7,
    ...
    {
      "title": "Async Pipe Patterns",
      "description": "The async pipe is Angular's most important pipe — handles Observable lifecycle.",
      "code": "// Observable + async = no memory leak\n@Component({\n  template: `\n    <!-- 1. Simple async -->\n    <p>{{ title$ | async }}</p>\n\n    <!-- 2. ngIf unwrap with alias -->\n    <div *ngIf=\"user$ | async as user\">\n      {{ user.name }}\n    </div>\n\n    <!-- 3. Multiple streams -->\n    <ng-container *ngIf=\"{\n      user: user$ | async,\n      orders: orders$ | async\n    } as vm\">\n      {{ vm.user?.name }} - {{ vm.orders?.length }} orders\n    </ng-container>\n  `\n})\nexport class ProfileComponent {\n  user$ = inject(UserService).currentUser$;\n  orders$ = inject(OrderService).userOrders$;\n}",
      "highlight": [
        5,
        8,
        9,
      ...
      "diagram": {
        ...
        "steps": [
          ...
          {
            "label": "Emits → template updates"
          },
          {
            "label": "Component destroyed"
          },
          {
            "label": "auto-unsubscribed"
          }
        ]
      }
    },
    {
      ...
      "diagram": {
        ...
        "steps": [
          ...
          {
            "label": "Pipe 1 transforms"
          },
          {
            "label": "Pipe 2 transforms"
          },
          {
            "label": "Final value"
          },
