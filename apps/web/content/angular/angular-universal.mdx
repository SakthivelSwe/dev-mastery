  ...
  "steps": [
    {
      ...
      "description": "Angular Universal renders pages on the server for better SEO and initial load performance.",
      "code": "# Add SSR\nng add @angular/ssr\n\n# Structure:\n# server.ts              - Express server\n# app.config.server.ts  - Server providers\n# app.config.ts         - Merged config\n\n// server.ts\nconst app = express();\n\napp.get('*', (req, res, next) => {\n  const { protocol, originalUrl, baseUrl, headers } = req;\n  commonEngine.render({\n    bootstrap, document, url: `${protocol}://${headers.host}${originalUrl}`\n  }).then(html => res.send(html))\n    .catch(err => next(err));\n});",
      "highlight": [
        2,
        10,
        12,
    ...
    },
    {
      ...
      "description": "SSR: some APIs don't exist on the server — guard browser-only code.",
      "code": "import { isPlatformBrowser, isPlatformServer } from '@angular/common';\nimport { PLATFORM_ID } from '@angular/core';\n\n@Injectable({ providedIn: 'root' })\nexport class StorageService {\n  private platformId = inject(PLATFORM_ID);\n\n  get(key: string): string | null {\n    if (!isPlatformBrowser(this.platformId)) return null;\n    return localStorage.getItem(key); // only runs in browser\n  }\n\n  set(key: string, value: string): void {\n    if (!isPlatformBrowser(this.platformId)) return;\n    localStorage.setItem(key, value);\n  }\n}",
      "highlight": [
        6,
        9,
        14
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "SSR runs on Node"
          },
          {
            "label": "isPlatformBrowser?"
          },
          {
            "label": "false → skip localStorage"
          },
          {
            "label": "Browser hydrates"
          },
          {
            "label": "true → localStorage ok"
          }
        ]
      }
    },
    {
      ...
      "description": "TransferState avoids duplicate HTTP calls — server fetches data, transfers to browser.",
      "code": "import { TransferState, makeStateKey } from '@angular/core';\n\nconst USERS_KEY = makeStateKey<User[]>('users');\n\n@Injectable({ providedIn: 'root' })\nexport class UserService {\n  private ts = inject(TransferState);\n  private http = inject(HttpClient);\n\n  getUsers(): Observable<User[]> {\n    const cached = this.ts.get(USERS_KEY, null);\n    if (cached) {\n      this.ts.remove(USERS_KEY);\n      return of(cached); // Browser: use server data\n    }\n    return this.http.get<User[]>('/api/users').pipe(\n      tap(users => this.ts.set(USERS_KEY, users)) // Server: store it\n    );\n  }\n}",
      "highlight": [
        3,
        11,
        12,
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "Server: HTTP fetch"
          },
          {
            "label": "ts.set() in HTML"
          },
          {
            "label": "Browser receives HTML"
          },
          {
            "label": "ts.get() reads"
          },
          {
            "label": "No second HTTP call"
          }
        ]
      }
    },
    {
      ...
      "diagram": {
        ...
        "steps": [
          ...
          {
            "label": "meta.updateTag()"
          },
          {
            "label": "SSR renders tags"
          },
          {
            "label": "Crawler sees tags"
          }
        ]
      }
    },
    {
      "title": "Hydration",
      "description": "Angular 17+ incremental hydration prevents flickering on client takeover.",
      ...
      "diagram": {
        ...
        "steps": [
          {
            "label": "Server HTML sent"
          },
          {
            "label": "provideClientHydration()"
          },
          {
            "label": "DOM reused (no flash)"
          },
          {
            "label": "Events replayed"
          },
          {
            "label": "hydrate on viewport"
          }
        ]
      }
    }
