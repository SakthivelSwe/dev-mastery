...
(async () => {
  await c.connect();
  const r = await c.query(`SELECT t.slug, COUNT(l.id) as lessons FROM topics t LEFT JOIN lessons l ON l.topic_id=t.id WHERE t.path_id=(SELECT id FROM learning_paths WHERE slug='java-mastery') GROUP BY t.slug ORDER BY lessons ASC, t.slug ASC`);
  console.log('Total topics:', r.rows.length);
  ...
  const total = r.rows.reduce((s, x) => s + Number(x.lessons), 0);
